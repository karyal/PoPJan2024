...
public class JDBCV5 {
	...
	public void insertRecord(Person person) {
		...
		final String sql = "insert into persons values(" + person.getPid() + ",'" + person.getFullname() + "', '"
	...
	public ArrayList selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				Person person = new Person(pid, fullName, address, email);
				persons.add(person);
			}
	...
	public void updateRecord(Person person) {
		...
		// final String sql = "UPDATE persons SET fullname='new name', address='new
		// address' WHERE pid=1";
		final String sql = "UPDATE persons SET fullname='" + person.getFullname() + "', address = '"
				+ person.getAddress() + "', email='" + person.getEmail() + "' WHERE pid =" + person.getPid();
		try {
			Class.forName(DRIVER); // Loading Driver
			Connection conn = DriverManager.getConnection(URL, USER, PASS);
			Statement stat = conn.createStatement();
